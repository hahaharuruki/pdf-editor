      ...
      <div class="row justify-content-center">
        <div class="col-md-6">
          <div class="card shadow-sm">
            <div class="card-body">
              <div class="mb-3">
                <input type="file" class="form-control d-none" id="pdfFiles" @change="onFileChange" accept="application/pdf" multiple ref="fileInput" />
                ...
                <div v-for="(file, index) in selectedFiles" :key="index" class="file-item">
                  ...
                  <div class="file-info">
                    ...
                    <div class="icon-group">
                      <i @click="moveFileUp(index)" class="fas fa-arrow-up text-primary" v-if="index > 0"></i>
                      <i @click="moveFileDown(index)" class="fas fa-arrow-down text-primary" v-if="index < selectedFiles.length - 1"></i>
                      <div class="spacer"></div>
                      <i @click="removeFile(index)" class="fas fa-trash-alt text-danger"></i>
                    </div>
                  </div>
                </div>
              </div>
              <button v-if="selectedFiles.length > 0 && !mergedPdfUrl" @click="mergePdfs" class="btn btn-primary w-100">2. 結合する</button>
              <a v-if="mergedPdfUrl" :href="mergedPdfUrl" download="merged.pdf" class="btn btn-secondary w-100 mt-2">3. ダウンロード</a>
            </div>
          </div>
        </div>
      </div>
